...
import { updateOffersForm } from "../../lib/actions";
...
export default function EditOffersUi({ productsData, offersData }) {
    ...
    useEffect(() => {
        setProductId(offersData.productId);
    }, [offersData]);
    ...
    const handleSubmit = async (e) => {
        setLoading(true);
        e.preventDefault();
        const result = await updateOffersForm(offersData.id, new FormData(e.target), navigate);
        setLoading(false);
    };
    ...
    if ((Object.keys(offersData).length < 1) || (productsData.length < 1)) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1  md:grid-cols-2 sm:grid-cols-1 gap-3">
                ...
                <div className="mb-4">
                    <label className="block text-black mb-2"> السعر الجديد</label>
                    <input
                        type="number"
                        defaultValue={offersData?.price}
                        name="final_price"
                ...
                <div className="mb-4">
                    <label className="block text-black mb-2">  المنتج</label>
                    <DropDown options={productsData} value={productId} onchange={onchange} defaultText="أختر منتج العرض" paramkey="product_id" />
                </div>
                <div className="mb-4">
                    ...
                    <input
                        type="date"
                        name="start_time"
                        defaultValue={offersData?.start_time}
                        className="w-full px-4 py-2 border border-orange rounded-lg focus:outline-none focus:ring-2 focus:ring-orange"
                        placeholder="أدخل التاريخ الجديد"
                    />
                </div>
                <div className="mb-4">
                    <label className="block text-black mb-2"> إلى التاريخ</label>
                    <input
                        type="date"
                        name="end_time"
                        defaultValue={offersData?.end_time}
                        className="w-full px-4 py-2 border border-orange rounded-lg focus:outline-none focus:ring-2 focus:ring-orange"
                        placeholder="أدخل التاريخ الجديد"
                    />
                </div>

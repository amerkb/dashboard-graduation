...
import { updateModelsForm } from "../../lib/actions";
...
export default function EditModelsUi({ brandData, modelsData }) {
    ...
    useEffect(() => {
        setBrandId(modelsData.brand_id);
    }, [modelsData]);
    ...
    const handleSubmit = async (e) => {
        setLoading(false);
        e.preventDefault();
        const result = await updateModelsForm(modelsData.id, new FormData(e.target), navigate);
        setLoading(false);
    };
    ...
    if ((Object.keys(modelsData).length < 1) || (brandData.length < 1)) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1  md:grid-cols-2 sm:grid-cols-1 gap-3">
                ...
                <div className="mb-4">
                    <label className="block text-black mb-2">اسم الموديل</label>
                    <input
                        type="text"
                        defaultValue={modelsData?.name}
                        name="name"
                ...
                <div className="mb-4">
                    ...
                    <DropDown options={brandData} value={brandId} onchange={onchange} paramkey={"brand_id"} defaultText="أختر الماركة" />
                </div>
            </div>
            {/* add dropdwon here  */}

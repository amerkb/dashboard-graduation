...
import { updateEmployeesForm } from "../../lib/actions";
...
export default function EditEmployeesUi({ roleData, employeeData }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        const result = await updateEmployeesForm(employeeData.id, new FormData(e.target), navigate);
        setLoading(false);
    };
    ...
    if ((Object.keys(employeeData).length < 1) || (roleData.length < 1)) {
            ...
            <div className="grid grid-cols-1  md:grid-cols-2 sm:grid-cols-1 gap-3">
                ...
                <div className="mb-4">
                    ...
                    <input
                        ...
                        name="password"
                        className="w-full px-4 py-2 border border-orange rounded-lg focus:outline-none focus:ring-2 focus:ring-orange"
                        placeholder="أدخل كلمة سر حساب الموظف"
                    />
                </div>
                <div className="mb-4">
                    ...
                    <input
                        ...
                        className="w-full px-4 py-2 border border-orange rounded-lg focus:outline-none focus:ring-2 focus:ring-orange"
                        placeholder="أدخل تاكيد كلمة سر حساب الموظف"
                    />
                </div>
                <div className="mb-4">
                    <label className="block text-black mb-2">اسم الوظيفة</label>
                    <DropDown options={roleData} value={roleId} onchange={onchange} paramkey={"role_id"} defaultText="أختر الوظيفة" />
                </div>
            </div>
            {/* add dropdwon here  */}

...
import { addProductsForm } from "../../lib/actions";
...
export default function AddProductsUi({ modelsData, drive_type, fuel_type, gears, seat_type, cylinders, brandsData, storesData, yearOptions, bodyType, type, handleBrandId }) {
    ...
    const handleSubmit = async (e) => {
        setLoading(true);
        e.preventDefault();
        const result = await addProductsForm(new FormData(e.target), navigate);
        setLoading(false);
    };
    ...
    if (((brandsData).length < 1) || (storesData.length < 1)) {

...
export default function Sidebar({ isOpen, toggleSidebar }) {
  ...
  // ✨ إغلاق كل الدروب داونز عند إغلاق السايدبار
  useEffect(() => {
    if (!isOpen) {
      setDropdownsOpen({});
    }
              ...
              {item.static && isOpen ? (
                ...
                <>
                  <SidebarItem
                    ...
                    onClick={() => {
                      if (isOpen) toggleDropdown(item.text);
                    }}
  ...
  return (
    <Component
      ...
      className={`group flex items-center p-3 rounded-xl transition-all duration-300 cursor-pointer 
        ...
        ${nested ? 'pl-6 text-sm' : 'text-base'} ${location.pathname === to
    ...
    >
      ...
      {isOpen && (
        <motion.span
          initial={{ opacity: 0, x: -10 }}
          animate={{ opacity: 1, x: 0 }}
          className={`mx-3 font-medium tracking-wide ${location.pathname === to ? 'text-white font-semibold' : 'text-white'
            }`}
        >
          {text}
        </motion.span>
      ...
      {hasDropdown && isOpen && (
        <motion.span
          ...
          className={`ml-auto transition-colors ${location.pathname === to ? 'text-orange-300' : 'text-white'
            }`}
        >
          <FaChevronDown />
        </motion.span>

...
function SidebarItem({
  ...
  return (
    ...
    >
      <span
        className={`transform transition-transform duration-300 text-white font-bold
             ${
               location.pathname === to
      ...
      {isOpen && (
        <motion.span
          initial={{ opacity: 0, x: -10 }}
          animate={{ opacity: 1, x: 0 }}
          className={`mx-3 font-medium tracking-wide ${
            location.pathname === to ? 'text-white font-semibold' : 'text-white'
          }`}
        >
          {text}
        </motion.span>
      ...
      {hasDropdown && isOpen && (
        <motion.span
          ...
          className={`ml-auto transition-colors ${
            location.pathname === to ? 'text-orange-300' : 'text-white'
          }`}
        >
          <FaChevronDown />
        </motion.span>
